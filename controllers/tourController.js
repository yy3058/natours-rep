...
exports.getTourStats = catchAsync(
  async (req, res, next) => {
    const stats = await Tour.aggregate([
      ...
      {
        $group: {
          _id: { $toUpper: '$difficulty' }, // groud by
          numTours: { $sum: 1 },
          numRatings: { $sum: 'ratingsQuantity' },
          avgRating: { $avg: '$ratingsAverage' },
          avgPrice: { $avg: '$price' },
          minPrice: { $min: '$price' },
          maxPrice: { $max: '$price' }, // $: operator
        },
      },
      {
        $sort: { avgPrice: 1 },
      ...
      // {
      //   $match: { _id: { $ne: 'EASY' } }, // exclude easy
      // },
    ]);
    res.status(200).json({
      status: 'success',
...
exports.getMonthlyPlan = catchAsync(
  async (req, res, next) => {
    ...
    const plan = await Tour.aggregate([
      ...
      {
        $group: {
          _id: { $month: '$startDates' }, // grop by month
          numTourStarts: { $sum: 1 }, // count
          tours: { $push: '$name' }, // push to array
        },
      },
      {
